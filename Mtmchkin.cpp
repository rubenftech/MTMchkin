    ...
    while (std::getline(source, tmp)) {
        try {
            if (tmp == "Gang") {
                ...
                createPlayCard<Mana>(gang_checker, index_of_line);

            } else if (tmp == "Gremlin") {
                createMonster<Gremlin>(gang_checker, index_of_line, tmp_array);

            } else if (tmp == "Merchant") {
                createPlayCard<Merchant>(gang_checker, index_of_line);

            } else if (tmp == "Well") {
                createPlayCard<Well>(gang_checker, index_of_line);

            } else if (tmp == "Treasure") {
                createPlayCard<Treasure>(gang_checker, index_of_line);

            } else if (tmp == "Witch") {
                createMonster<Witch>(gang_checker, index_of_line, tmp_array);
    ...
    for (int i=0;i<m_team_size;i++) {
        ...
        while (checker){
            ...
            if (!(validName(tmp_name))){
                 printInvalidName();
                 continue;
            }else if( ((tmp_class != "Ninja") && (tmp_class != "Healer") && (tmp_class != "Warrior"))){
              printInvalidClass();
                continue;
            }
            checker=false;
        ...
        if (tmp_class=="Ninja"){
            m_player.push_back(unique_ptr<Player>(new Ninja(tmp_name, i)));
        }
        else if(tmp_class=="Healer") {
            m_player.push_back(unique_ptr<Player>(new Healer(tmp_name, i)));
        }
        else if (tmp_class=="Warrior"){
            m_player.push_back(unique_ptr<Player>(new Warrior(tmp_name, i)));
        }
